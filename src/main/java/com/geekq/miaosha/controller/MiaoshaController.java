...
public class MiaoshaController implements InitializingBean {
    ...
    public ResultObject<Integer> miaosha(Model model, MiaoshaUser user, @PathVariable("path") String path,
        ...
        ResultObject<Integer> result = ResultObject.build();
...
//		}
    ...
    @ResponseBody
    public ResultObject<Long> miaoshaResult(Model model, MiaoshaUser user,
                                           @RequestParam("goodsId") long goodsId) {
    ...
    @ResponseBody
    public ResultObject<String> getMiaoshaPath(HttpServletRequest request, MiaoshaUser user,
                                              @RequestParam("goodsId") long goodsId,
                                              @RequestParam(value = "verifyCode", defaultValue = "0") int verifyCode
    ) {
        ResultObject<String> result = ResultObject.build();
        if (user == null) {
            result.withError(SESSION_ERROR.getCode(), SESSION_ERROR.getMessage());
            return result;
        }
    ...
    @ResponseBody
    public ResultObject<String> getMiaoshaVerifyCod(HttpServletResponse response
                                                  ) {
        ResultObject<String> result = ResultObject.build();
        try {
    ...
    @ResponseBody
    public ResultObject<String> getMiaoshaVerifyCod(HttpServletResponse response, MiaoshaUser user,
                                              @RequestParam("goodsId") long goodsId) {
        ResultObject<String> result = ResultObject.build();
        if (user == null) {
            result.withError(SESSION_ERROR.getCode(), SESSION_ERROR.getMessage());
